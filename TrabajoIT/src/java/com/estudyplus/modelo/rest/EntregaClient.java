...
public class EntregaClient {
    ...
    public EntregaClient() {
        ...
        webTarget = client.target(BASE_URI).path("rest.entrega");
    }
    ...
    /**
     * Obtiene una lista de entregas para un usuario específico.
     * Asume que el FacadeREST en el servidor tiene un endpoint similar a:
     * GET /rest.entrega/byUser/{userId}
     *
     * @param responseType El tipo de lista que se espera (e.g., List.class o una GenericType<List<Entrega>>).
     * @param userId El ID del usuario.
     * @param <T> El tipo de la respuesta.
     * @return Una lista de entregas.
    ...
    public <T> T findByUserId_JSON(GenericType<T> responseType, String userId) throws ClientErrorException {
        ...
        // Para obtener una List<Entrega> directamente, lo ideal sería:
        // return resource.request(MediaType.APPLICATION_JSON).get(new GenericType<List<Entrega>>() {});
    ...
    public <T> T getUpcomingByUserId_JSON(GenericType<T> responseType, String userId) throws ClientErrorException {
        WebTarget resource = webTarget;
        resource = resource.path(java.text.MessageFormat.format("usuario/{0}/proximas", new Object[]{userId})); // Ajusta el path de tu API REST
        return resource.request(javax.ws.rs.core.MediaType.APPLICATION_JSON).get(responseType);
    }
